...
#     training.seed=1103 training.device=cuda:1 hydra.run.dir="data/outputs/${name}_${task_name}" 
# python save_data.py --config-dir=diffusion_policy/configs_robomimic \
...
for folder in 'CFM' 'DER' 'logpZO' 'NatPN' 'PCA_kmeans' 'RND'
    do  
        if [ -d "UQ_baselines/$folder" ]; then
            ...
            python train.py --policy_type='diffusion' --type transport
            cd ../..
        else
...
# Step 4
cd UQ_test
python eval_together.py --policy_type='flow' --task_name='transport' --device=1 --modify=false --num=100
python eval_together.py --policy_type='diffusion' --task_name='transport' --device=1 --modify=false --num=100

python eval_together.py --policy_type='flow' --task_name='transport' --device=1 --modify=true --num=100
python eval_together.py --policy_type='diffusion' --task_name='transport' --device=1 --modify=true --num=100

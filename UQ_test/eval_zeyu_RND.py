...
class MultiTrajectoryFailureDetector:
    def __init__(self, task_name, policy_type, device='cuda'):
        ...
        # Load the trained RND model using the exact same logic as eval_load_baseline.py
        print(f"Loading RND model for {task_name} with {policy_type} policy...")
        self.RND_model = elb.get_baseline_model('RND', task_name, policy_type=policy_type).to(self.device)
        self.RND_model.eval()
        print("Model loaded successfully!")
        
        # Show reshaping info for RND
        ...
        # Set global_eps attribute for logpO compatibility (even though we're using RND)
        if not hasattr(self.RND_model, 'global_eps'):
            self.RND_model.global_eps = None
    ...
    def debug_rnd_output(self, observations, actions):
        """Debug function to understand what RND_UQ actually returns"""
        
        # Test with first timestep
        obs_t = torch.from_numpy(observations[0:1]).float().to(self.device)  # (1, 274)
        action_t = torch.from_numpy(actions[0:1]).float().to(self.device)    # (1, 160)
        
        # Reshape action to match training: (1, 16, 10)
        chunks_per_action = 16
        input_dim = 160 // chunks_per_action  # = 10
        action_reshaped = action_t.reshape(1, chunks_per_action, input_dim)
        
        print(f"Input shapes: action={action_reshaped.shape}, obs={obs_t.shape}")
        
        with torch.no_grad():
            # Direct model call
            print("=== Direct model call ===")
            try:
                raw_output = self.RND_model(action_reshaped, obs_t)
                print(f"Raw model output: {raw_output}")
                print(f"Raw output shape: {raw_output.shape}")
                print(f"Raw output dtype: {raw_output.dtype}")
                print(f"Raw output numel: {raw_output.numel()}")
            except Exception as e:
                print(f"Direct model call failed: {e}")
                return
                
            # RND_UQ function call
            print("\n=== RND_UQ function call ===")
            try:
                uq_output = elb.RND_UQ(self.RND_model, action_reshaped, obs_t)
                print(f"RND_UQ output: {uq_output}")
                print(f"RND_UQ output type: {type(uq_output)}")
                print(f"RND_UQ output shape: {uq_output.shape}")
                print(f"RND_UQ output dtype: {uq_output.dtype}")
                print(f"RND_UQ output numel: {uq_output.numel()}")
                
                # Try different ways to extract scalar
                print(f"uq_output[0]: {uq_output[0]}")
                print(f"uq_output[0].item(): {uq_output[0].item()}")
                print(f"uq_output.mean(): {uq_output.mean()}")
                print(f"uq_output.mean().item(): {uq_output.mean().item()}")
                
            except Exception as e:
                print(f"RND_UQ call failed: {e}")

    # Call this function before your main processing:

    # Then use this corrected version:
    def compute_failure_scores_single(self, observations, actions):
        """
        Corrected version based on debugging output
        """
        T, obs_dim = observations.shape
        T_act, act_dim = actions.shape
        
        chunks_per_action = 16
        input_dim = act_dim // chunks_per_action
        
        timestep_scores = []
        
        with torch.no_grad():
            for t in range(T):
                # Prepare inputs exactly like in the robomimic runner
                obs_t = torch.from_numpy(observations[t:t+1]).float().to(self.device)
                action_t = torch.from_numpy(actions[t:t+1].reshape(1, chunks_per_action, input_dim)).float().to(self.device)
                
                try:
                    # Use the exact same function as in the runner
                    score_tensor = elb.RND_UQ(self.RND_model, action_t, obs_t)
                    
                    # Handle the output based on what we learned from debugging
                    if score_tensor.numel() == 1:
                        score_value = score_tensor.item()
                    else:
                        # Take the first element if it's a batch
                        score_value = score_tensor[0].item()
                    
                    timestep_scores.append(score_value)
                    
                    # if t < 3:
                    #     print(f"t={t}: score_tensor.shape={score_tensor.shape}, value={score_value:.6f}")
                        
                except Exception as e:
                    print(f"Error at timestep {t}: {e}")
                    timestep_scores.append(0.0)
        
        return np.array(timestep_scores)
    ...
    def process_single_trajectory(self, file_path, threshold_method='percentile', percentile=95.0):
        ...
        # ADD THE DEBUG CALL HERE - only for the first trajectory
        # if filename.endswith('001') or 'episode0' in filename:  # Only debug first file
        #     print("\n=== DEBUGGING RND OUTPUT ===")
        #     self.debug_rnd_output(observations, actions)
        #     print("=== END DEBUG ===\n")
        
        # Compute failure scores
        scores = self.compute_failure_scores_single(observations, actions)

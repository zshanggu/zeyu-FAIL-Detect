...
class MultiTrajectoryFailureDetector:
    def __init__(self, task_name, policy_type, device='cuda'):
        ...
        # Load the trained CFM model using the exact same logic as eval_load_baseline.py
        print(f"Loading CFM model for {task_name} with {policy_type} policy...")
        self.CFM_model = elb.get_baseline_model('CFM', task_name, policy_type=policy_type).to(self.device)
        self.CFM_model.eval()
        print("Model loaded successfully!")
        
        # Show reshaping info for CFM
        ...
        # Set global_eps attribute for logpO compatibility (even though we're using CFM)
        if not hasattr(self.CFM_model, 'global_eps'):
            self.CFM_model.global_eps = None
    ...
    def compute_failure_scores_single(self, observations):
        """
        Compute CFM failure scores for a single trajectory
        ...
        with torch.no_grad():
            # Use the exact same CFM_UQ function from eval_load_baseline.py
            # This function:

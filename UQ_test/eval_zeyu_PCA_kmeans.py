...
class MultiTrajectoryFailureDetector:
    def __init__(self, task_name, policy_type, device='cuda'):
        ...
        # Load the trained PCA_kmeans model using the exact same logic as eval_load_baseline.py
        print(f"Loading PCA_kmeans model for {task_name} with {policy_type} policy...")
        self.PCA_kmeans_model = elb.get_baseline_model('PCA_kmeans', task_name, policy_type=policy_type).to(self.device)
        self.PCA_kmeans_model.eval()
        print("Model loaded successfully!")
        
        # Show reshaping info for PCA_kmeans
        in_dim = 10 if task_name != 'transport' else 20
        print(f"\nPCA_kmeans model expects:")
        print(f"  Input chunks of size: {in_dim}")
        print(f"  Your 274D observations will be reshaped accordingly")
        
        # Set global_eps attribute for logpO compatibility (even though we're using PCA_kmeans)
        if not hasattr(self.PCA_kmeans_model, 'global_eps'):
            self.PCA_kmeans_model.global_eps = None
    ...
    def compute_failure_scores_single(self, observations):
        """
        Compute PCA_kmeans failure scores for a single trajectory
        ...
        with torch.no_grad():
            # Use the exact same PCA_kmeans_UQ function from eval_load_baseline.py
            # This function:
            ...
            scores = elb.PCA_kmeans_UQ(
                baseline_model=self.PCA_kmeans_model,

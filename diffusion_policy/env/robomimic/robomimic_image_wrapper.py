...
class RobomimicImageWrapper(gym.Env):
    ...
    def modify_environment(self, delta=0.1, render_obs_key='agentview_image'):
        ...
        simul = self.env.env.sim  # Adjusted access path
        camera_modder = CameraModder(sim=simul)
        camera_name_dict = {'agentview_image': 'agentview',
                            'shouldercamera0_image': 'shouldercamera0',
                            'sideview_image': 'sideview'}
        camera_name = camera_name_dict[render_obs_key]
        pos = camera_modder.get_pos(camera_name)
        print(f"delta: {delta} at camera: {camera_name}")
        camera_modder.set_pos(camera_name, np.array([pos[0] + delta, pos[1], pos[2]]))
...
def test():
    ...
    dataset_path = os.path.expanduser('~/dev/diffusion_policy//home/chenxu/Downloads/TRI_chen/sim_eval/data/robomimic/datasets/square/ph/image.hdf5')
